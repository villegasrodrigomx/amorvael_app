document.addEventListener('DOMContentLoaded', () => {
  ...
  function initializeCalendar(serviceId, view) {
    ...
    let hoyOficial = ''; // Guardaremos aquí el "hoy" del servidor
    ...
    function renderCalendar() {
      ...
      for (let day = 1; day <= lastDayOfMonth.getDate(); day++) {
        ...
        // --- REGLA DE NEGOCIO: DESHABILITAR HOY Y DÍAS PASADOS ---
        if (isoDate <= hoyOficial) {
    ...
    async function fetchAndDisplaySlots(serviceId, date) {
        ...
        try {
            ...
            if (data.status === 'success') {
                // Guardamos el "hoy" oficial la primera vez que lo recibimos
                if (!hoyOficial) {
                    hoyOficial = data.hoy;
                    // Volvemos a renderizar el calendario para aplicar la regla de deshabilitar hoy
                    renderCalendar(); 
                }
    ...
    // El resto de la función y del archivo no cambia.
    // ...
  }
  
  // --- CÓDIGO RESTANTE (sin cambios, para copiar y pegar todo) ---
  function initializeCalendar(serviceId, view) { let currentDate = new Date(); let occupiedDates = []; let selection = { checkIn: null, checkOut: null }; let currentPrice = 0; let hoyOficial = ''; const monthYearEl = view.querySelector('#monthYear'); const calendarDaysEl = view.querySelector('#calendarDays'); const prevMonthBtn = view.querySelector('#prevMonth'); const nextMonthBtn = view.querySelector('#nextMonth'); const slotsContainer = view.querySelector('.slots-container'); const availableSlotsEl = view.querySelector('#availableSlots'); function renderCalendar() { const month = currentDate.getMonth(); const year = currentDate.getFullYear(); monthYearEl.textContent = currentDate.toLocaleDateString('es-MX', { month: 'long', year: 'numeric' }); calendarDaysEl.innerHTML = ''; const firstDayOfMonth = new Date(year, month, 1); const lastDayOfMonth = new Date(year, month + 1, 0); const startDayOfWeek = firstDayOfMonth.getDay(); for (let i = 0; i < startDayOfWeek; i++) calendarDaysEl.insertAdjacentHTML('beforeend', '<div class="calendar-day disabled"></div>'); for (let day = 1; day <= lastDayOfMonth.getDate(); day++) { const dayCell = document.createElement('div'); const dayDate = new Date(year, month, day); const isoDate = toISODateString(dayDate); dayCell.className = 'calendar-day'; dayCell.textContent = day; if (isoDate <= hoyOficial) { dayCell.classList.add('disabled'); } else { dayCell.addEventListener('click', async () => { view.querySelectorAll('.calendar-day.selected').forEach(el => el.classList.remove('selected')); dayCell.classList.add('selected'); await fetchAndDisplaySlots(serviceId, dayDate); }); } calendarDaysEl.appendChild(dayCell); } } async function fetchAndDisplaySlots(serviceId, date) { slotsContainer.style.display = 'block'; availableSlotsEl.innerHTML = ''; availableSlotsEl.appendChild(document.getElementById('template-loading').content.cloneNode(true)); const dateString = toISODateString(date); const url = `${API_ENDPOINT}?action=getAvailableSlots&serviceId=${serviceId}&date=${dateString}`; try { const response = await fetch(url); if (!response.ok) { const errorText = await response.text(); throw new Error(`El servidor respondió con un error. Respuesta: ${errorText}`); } const data = await response.json(); availableSlotsEl.innerHTML = ''; if (data.status === 'success') { if (!hoyOficial) { hoyOficial = data.hoy; renderCalendar(); } if (data.availableSlots.length > 0) { data.availableSlots.sort().forEach(slotTime24h => { const slotEl = document.createElement('div'); slotEl.className = 'slot'; slotEl.textContent = formatTime12h(slotTime24h); slotEl.addEventListener('click', () => openBookingModal(serviceId, date, slotTime24h)); availableSlotsEl.appendChild(slotEl); }); } else { availableSlotsEl.innerHTML = '<p>No hay horarios disponibles para este día.</p>'; } } else { throw new Error(data.message); } } catch (error) { availableSlotsEl.innerHTML = `<p class="error-message">No se pudo cargar la disponibilidad.</p>`; } } prevMonthBtn.addEventListener('click', () => { currentDate.setMonth(currentDate.getMonth() - 1); renderCalendar(); }); nextMonthBtn.addEventListener('click', () => { currentDate.setMonth(currentDate.getMonth() + 1); renderCalendar(); }); renderCalendar(); fetchAndDisplaySlots(serviceId, new Date(new Date().setDate(new Date().getDate() + 1))); } /* Inicializa con mañana */
  
  function openBookingModal(serviceId, date, time24h) { const service = allData.services.find(s => s.id === serviceId); const modal = document.getElementById('booking-modal'); document.getElementById('modal-service-name').textContent = service.nombre; document.getElementById('modal-date').textContent = date.toLocaleDateString('es-MX', { weekday: 'long', year: 'numeric', month: 'long', day: 'numeric' }); document.getElementById('modal-time').textContent = formatTime12h(time24h); modal.style.display = 'flex'; document.getElementById('close-modal').onclick = () => modal.style.display = 'none'; const confirmBtn = document.getElementById('confirm-booking-btn'); confirmBtn.onclick = async () => { const clientName = document.getElementById('clientName').value; const clientEmail = document.getElementById('clientEmail').value; const clientPhone = document.getElementById('clientPhone').value; if (!clientName || !clientEmail || !clientPhone) { alert('Por favor, completa todos los campos.'); return; } confirmBtn.textContent = 'Procesando...'; confirmBtn.disabled = true; const bookingData = { serviceId, date: `${date.getFullYear()}-${String(date.getMonth() + 1).padStart(2, '0')}-${String(date.getDate()).padStart(2, '0')}`, time: time24h, clientName, clientEmail, clientPhone }; const modalMessage = document.getElementById('modal-message'); try { const response = await fetch(API_ENDPOINT, { method: 'POST', body: JSON.stringify(bookingData) }); const result = await response.json(); if (result.status === 'success') { modalMessage.textContent = result.message; modalMessage.className = 'success'; document.querySelector('#booking-modal .booking-form').style.display = 'none'; } else { throw new Error(result.message); } } catch (error) { modalMessage.textContent = error.message; modalMessage.className = 'error'; confirmBtn.textContent = 'Confirmar Cita'; confirmBtn.disabled = false; } modalMessage.style.display = 'block'; }; }
