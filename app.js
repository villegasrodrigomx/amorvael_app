document.addEventListener('DOMContentLoaded', () => {
  ...
  // --- VISTA: MIS PAQUETES (ACTUALIZADA) ---
  ...
  // --- VISTA: SERVICIOS DE UN PAQUETE (ACTUALIZADA) ---
  async function renderPackageServicesView(purchaseId) {
    ...
    try {
      ...
      // Mostramos solo los servicios que quedan en el paquete
  ...
  function renderView(templateId) { 
    ...
    if (!template) { console.error(`La plantilla con ID "${templateId}" no fue encontrada.`); 
  ...
  async function renderCategoriesView() {
    ...
    try { if (!allData) allData = await fetchAppData();
         const categories = [...new Set(allData.services.map(s => s.categoria))];
         categoryGrid.innerHTML = '';
         categories.forEach(categoryName => {const card = createCard('category', { name: categoryName });
         card.addEventListener('click', (e) => { e.preventDefault(); navigateTo(`?category=${encodeURIComponent(categoryName)}`); });
         categoryGrid.appendChild(card); }); if (allData.packages && allData.packages.length > 0) { const packageCard = createCard('package'); packageCard.addEventListener('click', (e) => { e.preventDefault(); navigateTo('?view=packages'); }); categoryGrid.appendChild(packageCard); } } catch (error) { categoryGrid.innerHTML = `<p class="error-message">Error al cargar las categorías: ${error.message}</p>`; } }
  ...
  async function renderServiceDetailView(serviceId, purchaseId = null) { const view = renderView('template-service-detail-view'); if (!view) return; view.prepend(document.getElementById('template-loading').content.cloneNode(true)); try { if (!allData) allData = await fetchAppData(); const service = allData.services.find(s => s.id === serviceId); if (!service) throw new Error('Servicio no encontrado.'); view.querySelector('.loading-spinner')?.remove(); const category = service.categoria || ''; view.querySelector('.back-link').addEventListener('click', (e) => { e.preventDefault(); if (purchaseId) { navigateTo(`?view=book-package-session&purchaseId=${purchaseId}`); } else { navigateTo(`?category=${encodeURIComponent(category)}`); } }); view.querySelector('.service-main-image').src = service.imagenUrl || getCategoryImage(category); view.querySelector('.view-title').textContent = service.nombre; view.querySelector('.service-price').textContent = `$${service.precio.toLocaleString('es-MX')} MXN`; view.querySelector('.service-duration').textContent = `Duración: ${service.duracion} minutos`; view.querySelector('.service-description').textContent = service.descripcion || 'Descripción no disponible.'; const showCalendarBtn = view.querySelector('#show-calendar-btn'); const bookingSection = view.querySelector('.booking-section'); showCalendarBtn.addEventListener('click', () => { bookingSection.style.display = 'block'; showCalendarBtn.style.display = 'none'; initializeCalendar(serviceId, view, purchaseId); }); } catch (error) { view.innerHTML = `<p class="error-message">Error al cargar el servicio: ${error.message}</p>`; } }

  async function renderPackageDetailView(packageId) { const view = renderView('template-package-detail-view'); if (!view) return; view.querySelector('.back-link').addEventListener('click', (e) => { e.preventDefault(); navigateTo('?view=packages'); }); view.prepend(document.getElementById('template-loading').content.cloneNode(true)); try { if (!allData) allData = await fetchAppData(); const pkg = allData.packages.find(p => p.id === packageId); if (!pkg) throw new Error('Paquete no encontrado.'); view.querySelector('.loading-spinner')?.remove(); view.querySelector('.view-title').textContent = pkg.nombre; view.querySelector('.package-price').textContent = `$${pkg.precio.toLocaleString('es-MX')} MXN`; const servicesIncludedList = view.querySelector('.package-services-included ul'); servicesIncludedList.innerHTML = ''; pkg.servicios.forEach(serviceId => { const service = allData.services.find(s => s.id === serviceId); if (service) { const listItem = document.createElement('li'); listItem.textContent = service.nombre; servicesIncludedList.appendChild(listItem); } }); view.querySelector('#buy-package-btn').addEventListener('click', () => { openPurchaseModal(pkg); }); } catch (error) { view.innerHTML = `<p class="error-message">Error al cargar el paquete: ${error.message}</p>`; } }

  function initializeCalendar(serviceId, view, purchaseId = null) { const monthYearEl = view.querySelector('#monthYear'); const calendarDaysEl = view.querySelector('#calendarDays'); const prevMonthBtn = view.querySelector('#prevMonth'); const nextMonthBtn = view.querySelector('#nextMonth'); const slotsContainer = view.querySelector('.slots-container'); const availableSlotsEl = view.querySelector('#availableSlots'); let currentDate = new Date(); let hoyOficial = ''; function renderCalendar() { const month = currentDate.getMonth(); const year = currentDate.getFullYear(); monthYearEl.textContent = currentDate.toLocaleDateString('es-MX', { month: 'long', year: 'numeric' }); calendarDaysEl.innerHTML = ''; const firstDayOfMonth = new Date(year, month, 1); const lastDayOfMonth = new Date(year, month + 1, 0); const startDayOfWeek = firstDayOfMonth.getDay(); for (let i = 0; i < startDayOfWeek; i++) calendarDaysEl.insertAdjacentHTML('beforeend', '<div class="calendar-day disabled"></div>'); for (let day = 1; day <= lastDayOfMonth.getDate(); day++) { const dayCell = document.createElement('div'); const dayDate = new Date(year, month, day); const isoDate = toISODateString(dayDate); dayCell.className = 'calendar-day'; dayCell.textContent = day; if (hoyOficial && isoDate <= hoyOficial) { dayCell.classList.add('disabled'); } else { dayCell.addEventListener('click', async () => { view.querySelectorAll('.calendar-day.selected').forEach(el => el.classList.remove('selected')); dayCell.classList.add('selected'); await fetchAndDisplaySlots(serviceId, dayDate, purchaseId); }); } calendarDaysEl.appendChild(dayCell); } } async function fetchAndDisplaySlots(serviceId, date, purchaseId) { slotsContainer.style.display = 'block'; availableSlotsEl.innerHTML = ''; availableSlotsEl.appendChild(document.getElementById('template-loading').content.cloneNode(true)); const dateString = toISODateString(date); const url = `${API_ENDPOINT}?action=getAvailableSlots&serviceId=${serviceId}&date=${dateString}`; try { const response = await fetch(url); const data = await response.json(); availableSlotsEl.innerHTML = ''; if (data.status === 'success') { if (data.availableSlots.length > 0) { data.availableSlots.sort().forEach(slotTime24h => { const slotEl = document.createElement('div'); slotEl.className = 'slot'; slotEl.textContent = formatTime12h(slotTime24h); slotEl.addEventListener('click', () => openBookingModal(serviceId, date, slotTime24h, purchaseId)); availableSlotsEl.appendChild(slotEl); }); } else { availableSlotsEl.innerHTML = '<p>No hay horarios disponibles para este día.</p>'; } } else { throw new Error(data.message); } } catch (error) { availableSlotsEl.innerHTML = `<p class="error-message">No se pudo cargar la disponibilidad.</p>`; } } async function setupCalendar() { const tomorrow = new Date(); tomorrow.setDate(tomorrow.getDate() + 1); const dateString = toISODateString(tomorrow); const url = `${API_ENDPOINT}?action=getAvailableSlots&serviceId=${serviceId}&date=${dateString}`; try { const response = await fetch(url); const data = await response.json(); if (data.status === 'success') { hoyOficial = data.hoy; renderCalendar(); } else { throw new Error(data.message); } } catch(error) { console.error("Error al inicializar el calendario:", error); calendarDaysEl.innerHTML = `<p class="error-message">No se pudo inicializar el calendario.</p>`; } } prevMonthBtn.addEventListener('click', () => { currentDate.setMonth(currentDate.getMonth() - 1); renderCalendar(); }); nextMonthBtn.addEventListener('click', () => { currentDate.setMonth(currentDate.getMonth() + 1); renderCalendar(); }); setupCalendar(); }

  function openBookingModal(serviceId, date, time24h, purchaseId = null) { if (purchaseId && !clientData) { alert("Tu sesión ha expirado. Por favor, identifícate de nuevo para agendar tu sesión."); navigateTo('?view=client-login'); return; } const service = allData.services.find(s => s.id === serviceId); const modal = document.getElementById('booking-modal'); document.querySelector('#booking-modal .booking-form').style.display = 'block'; const modalMessage = document.getElementById('modal-message'); modalMessage.style.display = 'none'; const confirmBtn = document.getElementById('confirm-booking-btn'); confirmBtn.disabled = false; document.getElementById('modal-service-name').textContent = service.nombre; document.getElementById('modal-date').textContent = date.toLocaleDateString('es-MX', { weekday: 'long', year: 'numeric', month: 'long', day: 'numeric' }); document.getElementById('modal-time').textContent = formatTime12h(time24h); const clientNameInput = document.getElementById('clientName'); const clientEmailInput = document.getElementById('clientEmail'); const clientPhoneInput = document.getElementById('clientPhone'); if (purchaseId) { document.getElementById('modal-title').textContent = 'Confirma tu Sesión de Paquete'; document.getElementById('modal-price').textContent = 'Incluido en tu paquete'; confirmBtn.textContent = 'Confirmar Sesión'; const purchase = clientData.packages.find(p => p.id === purchaseId); clientNameInput.value = purchase.nombreCliente; clientEmailInput.value = purchase.email; clientPhoneInput.value = purchase.telefono || ''; } else { document.getElementById('modal-title').textContent = 'Revisa y Confirma tu Cita'; document.getElementById('modal-price').textContent = `$${service.precio.toLocaleString('es-MX')} MXN`; confirmBtn.textContent = 'Confirmar Cita'; clientNameInput.value = ''; clientEmailInput.value = ''; clientPhoneInput.value = ''; } modal.style.display = 'flex'; document.getElementById('close-modal').onclick = () => modal.style.display = 'none'; confirmBtn.onclick = async () => { confirmBtn.textContent = 'Procesando...'; confirmBtn.disabled = true; let bookingData; if (purchaseId) { bookingData = { action: 'bookPackageSession', serviceId, date: toISODateString(date), time: time24h, clientEmail: clientEmailInput.value, purchaseId }; } else { bookingData = { action: 'createBooking', serviceId, date: toISODateString(date), time: time24h, clientName: clientNameInput.value, clientEmail: clientEmailInput.value, clientPhone: clientPhoneInput.value }; } try { const response = await fetch(API_ENDPOINT, { method: 'POST', body: JSON.stringify(bookingData) }); const result = await response.json(); if (result.status === 'success') { document.getElementById('modal-title').textContent = '¡Cita Confirmada!'; modalMessage.textContent = result.message; modalMessage.className = 'success'; document.querySelector('#booking-modal .booking-form').style.display = 'none'; if(purchaseId) { sessionStorage.removeItem('amorVaelClientData'); clientData = null; } } else { throw new Error(result.message); } } catch (error) { modalMessage.textContent = error.message; modalMessage.className = 'error'; confirmBtn.textContent = purchaseId ? 'Confirmar Sesión' : 'Confirmar Cita'; confirmBtn.disabled = false; } modalMessage.style.display = 'block'; }; }

  function openPurchaseModal(pkg) { const modal = document.getElementById('booking-modal'); document.getElementById('modal-title').textContent = 'Confirmar Compra de Paquete'; document.querySelector('#booking-modal .booking-summary').innerHTML = `<p><strong>Paquete:</strong> <span id="modal-service-name">${pkg.nombre}</span></p><p><strong>Precio:</strong> <span id="modal-price">$${pkg.precio.toLocaleString('es-MX')} MXN</span></p>`; document.querySelector('#booking-modal .booking-form').style.display = 'block'; const modalMessage = document.getElementById('modal-message'); modalMessage.style.display = 'none'; const confirmBtn = document.getElementById('confirm-booking-btn'); confirmBtn.disabled = false; confirmBtn.textContent = 'Confirmar Compra'; modal.style.display = 'flex'; document.getElementById('close-modal').onclick = () => modal.style.display = 'none'; confirmBtn.onclick = async () => { const clientName = document.getElementById('clientName').value; const clientEmail = document.getElementById('clientEmail').value; const clientPhone = document.getElementById('clientPhone').value; if (!clientName || !clientEmail || !clientPhone) { alert('Por favor, completa todos los campos.'); return; } confirmBtn.textContent = 'Procesando...'; confirmBtn.disabled = true; const purchaseData = { action: 'purchasePackage', packageId: pkg.id, clientName, clientEmail, clientPhone }; try { const response = await fetch(API_ENDPOINT, { method: 'POST', body: JSON.stringify(purchaseData) }); const result = await response.json(); if (result.status === 'success') { document.getElementById('modal-title').textContent = '¡Compra Exitosa!'; modalMessage.textContent = result.message; modalMessage.className = 'success'; document.querySelector('#booking-modal .booking-form').style.display = 'none'; } else { throw new Error(result.message); } } catch (error) { modalMessage.textContent = error.message; modalMessage.className = 'error'; confirmBtn.textContent = 'Confirmar Compra'; confirmBtn.disabled = false; } modalMessage.style.display = 'block'; }; }
  ...
  function formatTime12h(time24h) { if (!time24h) return ''; const [hours, minutes] = time24h.split(':'); const h = parseInt(hours, 10); const suffix = h >= 12 ? 'p.m.' : 'a.m.'; const hour12 = ((h + 11) % 12 + 1); return `${hour12}:${minutes} ${suffix}`; }
