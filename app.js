/**
 * Motor de Citas Amor-Vael v21.2 - VERSIÓN FINAL CON OPCIONES DE PAGO
 * - CORREGIDO: Se restaura y habilita la lógica para seleccionar el método de pago (tarjeta, transferencia, efectivo).
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  function renderCategoriesView() {
      ...
      grid.innerHTML = categories.map(category => {
          let item = category === 'Paquetes Especiales' ? { imagen: 'http://amor-vael.com/wp-content/uploads/2021/08/paquetes.jpg' } : allData.allServices.find(s => s.categoria === category) || allData.allPackages.find(p => p.categoria === category);
          return `<div class="category-card" data-category="${encodeURIComponent(category)}"><img src="${item.imagen}" alt="${category}"><h3>${category}</h3></div>`;
      }).join('');
  ...
  function renderServicesView(category) {
      ...
      const items = category === 'Paquetes Especiales' ? allData.allPackages : [...(allData.allServices || []), ...(allData.allPackages || [])].filter(item => item.categoria === category);
  ...
  function renderPackageDetailView(packageId) {
      // ... (Tu lógica original de renderizado de paquetes intacta aquí)
  ...
  function renderServiceDetailView(serviceId) {
      ...
      // Configurar listeners de descuentos y disponibilidad
      document.getElementById('apply-service-discount-btn').onclick = () => applyDiscount(service.id, service.precio, 'service');
      const dateInput = document.getElementById('booking-date');
      dateInput.onchange = async () => { /* ... Tu lógica de búsqueda de horarios ... */ const date = dateInput.value; const slotsContainer = document.getElementById('available-slots-container'); slotsContainer.innerHTML = 'Buscando horarios...'; document.querySelectorAll('.slot-button').forEach(b => b.classList.remove('selected')); try { const response = await fetch(`${API_ENDPOINT}?action=getAvailableSlots&serviceId=${serviceId}&date=${date}`); const result = await response.json(); if (result.status === 'success' && result.availableSlots.length > 0) { slotsContainer.innerHTML = result.availableSlots.map(slot => `<button class="slot-button">${slot}</button>`).join(''); } else { slotsContainer.innerHTML = 'No hay horarios disponibles para esta fecha.'; } } catch (error) { slotsContainer.innerHTML = 'Error al buscar horarios.'; } };
      document.getElementById('available-slots-container').addEventListener('click', e => { if (e.target.classList.contains('slot-button')) { document.querySelectorAll('.slot-button').forEach(b => b.classList.remove('selected')); e.target.classList.add('selected'); } });

      // *** LÓGICA DE OPCIONES DE PAGO RESTAURADA Y MEJORADA ***
      const paymentOptions = document.querySelectorAll('input[name="payment-method"]');
      const transferDetails = document.getElementById('transfer-details');
      const paymentSection = document.getElementById('payment-section'); // Stripe
  ...
  async function applyDiscount(itemId, originalPrice, typePrefix) { /* ... Tu lógica de descuentos intacta ... */ const code = document.getElementById(`${typePrefix}-discount-code`).value; const messageEl = document.getElementById(`${typePrefix}-discount-message`); const finalPriceEl = document.getElementById(`${typePrefix}-final-price`); if (!code) { messageEl.textContent = 'Ingresa un código.'; messageEl.className = 'error'; return; } messageEl.textContent = 'Validando...'; try { const response = await fetch(`${API_ENDPOINT}?action=validateDiscountCode&code=${code}&itemId=${itemId}`); const result = await response.json(); if (result.status !== 'success') throw new Error(result.message); currentDiscount = result.discount; const newPrice = calculateDiscountedPrice(originalPrice, currentDiscount); finalPriceEl.textContent = newPrice.toLocaleString('es-MX'); messageEl.textContent = '¡Descuento aplicado!'; messageEl.className = 'success'; } catch (error) { finalPriceEl.textContent = originalPrice.toLocaleString('es-MX'); messageEl.textContent = error.message; messageEl.className = 'error'; } }
  function calculateDiscountedPrice(originalPrice, discount) { /* ... Tu lógica de descuentos intacta ... */ const value = parseFloat(discount.Valor); if (discount.Tipo === '%') return originalPrice * (1 - value / 100); if (discount.Tipo === 'MXN') return Math.max(0, originalPrice - value); return originalPrice; }
  
  // =================================================================
  // DELEGACIÓN DE EVENTOS (Navegación principal)
  // =================================================================
