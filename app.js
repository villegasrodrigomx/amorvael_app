document.addEventListener('DOMContentLoaded', () => {
  ...
  let allData = null;
  let clientData = null; // Para guardar los datos del cliente logueado
  ...
  async function renderClientPackagesView() {
    ...
    if (!clientData || clientData.packages.length === 0) {
      listEl.innerHTML = '<p>No tienes paquetes activos. <a href="?view=client-login">Intenta con otro correo</a>.</p>';
      return;
    }
    listEl.innerHTML = '';
  ...
  async function renderPackageServicesView(purchaseId) {
    ...
    try {
      ...
      if (!clientData) { // Si se recarga la página, volvemos al login
  ...
  function openBookingModal(serviceId, date, time24h, purchaseId = null) {
    ...
    if (purchaseId) { // Si es una cita de paquete
      ...
      // Pre-llenamos los datos del cliente
      const purchase = clientData.packages.find(p => p.id === purchaseId);
    ...
    confirmBtn.onclick = async () => {
      // ... (lógica de confirmación, ahora envía 'action' y 'purchaseId' si es necesario)
      ...
      try {
        ...
        if (result.status === 'success') {
          ...
          // Actualizar datos del cliente si es una sesión de paquete
          if(purchaseId) clientData = null;
        } else {
