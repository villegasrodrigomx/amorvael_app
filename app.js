document.addEventListener('DOMContentLoaded', () => {
  ...
  let allData = null;
  // --- CORRECCIÓN CLAVE ---
  // Intentamos cargar los datos del cliente desde la memoria del navegador al iniciar.
    ...
    view.querySelector('#find-packages-btn').addEventListener('click', async () => {
      ...
      try {
        ...
        if (data.status === 'success' && data.clientPackages.length > 0) {
          // --- CORRECCIÓN CLAVE ---
          // Guardamos los datos del cliente en la memoria del navegador.
  ...
  async function renderClientPackagesView() {
    ...
    clientData.packages.forEach(pkg => {
      ...
      card.innerHTML = `
        ...
        <p><strong>Sesiones restantes:</strong> ${pkg.sesionesRestantes}</p>
      `;
  ...
  // --- LÓGICA DEL MODAL DE RESERVA (ACTUALIZADA) ---
  function openBookingModal(serviceId, date, time24h, purchaseId = null) {
    // --- CORRECCIÓN CLAVE ---
    // Si es una cita de paquete pero no "recordamos" al cliente, lo enviamos al login.
    ...
    confirmBtn.onclick = async () => {
        ...
        try {
            ...
            if (result.status === 'success') {
                ...
                if(purchaseId) {
                  sessionStorage.removeItem('amorVaelClientData'); // Limpiamos la sesión para forzar la recarga de datos
                  clientData = null;
                }
            } else {
