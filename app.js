/**
 * Motor de Citas Amor-Vael v24.1 - VERSIÓN FINAL Y ESTABLE BASADA EN ORIGINALES
 * - Se restaura y respeta la lógica del archivo app.js original del cliente.
 * - ROUTER CORREGIDO: Se repara la lógica para que la página de inicio siempre cargue las categorías,
 * evitando el error de "contenedor roto".
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  function router() {
    ...
    // Lógica a prueba de fallos para la carga inicial
    if (packageId) {
  ...
  async function loadInitialData() {
    ...
    try {
      ...
      if (result.status === 'success') {
        allData = result; // La estructura original esperaba el objeto completo {status, services, packages}
        router();
  ...
  function createCard(type, data) {
    const card = document.createElement('div'); // Usamos div para poder añadirle un listener
    
    if (type === 'category') {
      card.className = 'category-card';
      card.onclick = () => navigateTo(`?category=${encodeURIComponent(data.name)}`);
      ...
      card.className = 'category-card';
      card.onclick = () => navigateTo('?category=Paquetes Especiales');
      ...
      card.onclick = () => navigateTo(`?service=${data.id}`);
      ...
      card.onclick = () => navigateTo(`?packageId=${data.id}`);
